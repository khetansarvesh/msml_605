...
def main():
    """
    Main function to train and evaluate various N-gram language models and generate text.
    Steps:
        1. Train and evaluate MLE models (unigram, bigram, trigram, 4-gram)
        2. Train and evaluate Add-1 smoothed trigram model
        3. Train and evaluate linear interpolation model (with optimal lambdas)
        4. Train and evaluate Stupid Backoff model (with optimal alpha)
        5. Generate sample sentences using the best model
    """
    train_text, dev_text, test_text = load_penn_treebank_data()

    # 1. MLE Models (N=1,2,3,4)
